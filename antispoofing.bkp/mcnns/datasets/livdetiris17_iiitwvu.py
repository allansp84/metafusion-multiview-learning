...
class LivDetIris17_IIITWVU(Dataset):
    ...
    def __init__(self, dataset_path, ground_truth_path='', permutation_path='', iris_location='',
        ...
        super(LivDetIris17_IIITWVU, self).__init__(dataset_path, output_path, iris_location, file_types, operation, max_axis)

        self.sets = {}

        self.verbose = True

    def saving_ground_truth(self, inpath, all_fnames, all_labels, indexes, output_fname):

        labels = all_labels[indexes]

        fnames = []
        for fname in all_fnames[indexes]:
            fnames += [os.path.relpath(fname, inpath)]
    ...
    def build_meta(self, inpath, filetypes):
        ...
        for i, folder in enumerate(folders):
            ...
            for j, fname in enumerate(fnames):
                ...
                label = 0 if 'Live/' in rel_path else 1
        ...
        # self.saving_ground_truth(inpath, all_fnames, all_labels, train_idxs, output_fname='livdet-iiitwvu-train.txt')
        # self.saving_ground_truth(inpath, all_fnames, all_labels, test_idxs, output_fname='livdet-iiitwvu-test.txt')
        ...
        r_dict = {'all_fnames': all_fnames,
                  ...
                  'test_idxs': {'test': test_idxs,
                                'unknown_test': test_idxs,
                                'overall_test': test_idxs,
                                },
    ...
    def protocol_eval(self, fold=0, n_fold=5, test_size=0.5, output_path=''):
        ...
        create_mosaic(all_data[all_pos_idxs][:20], n_col=10, output_fname=os.path.join(output_path, 'mosaic-iiitwvu-pos.png'))
        create_mosaic(all_data[all_neg_idxs][:20], n_col=10, output_fname=os.path.join(output_path, 'mosaic-iiitwvu-neg.png'))
        ...
        for test_id in test_idxs:
            if test_idxs[test_id].size:
                test_set[test_id] = {'data': all_data[[test_id]],
